...
def main(trn_file_path, output_dir, mode='tokenized_kana', wav_file_path=None,
    ...
    """TRNファイルを読み込んで, テキストファイルを出力する

    Args:
        trn_file_path (str): TRNファイルのパス
        output_dir (str): 出力先のディレクトリ
        mode (str): 出力するテキストの形式. 'plain', 'kana', 'tokenized_kana' のいずれか
        encoding (str, optional): TRNファイルのエンコーディング. Defaults to 'shift_jis'.
        gap (float, optional): 発話を接続する際の最大の間隔. Defaults to 0.5.
        max (float, optional): 1つの発話に含める最大の時間. Defaults to 10.0.
        remove_extra_content (bool, optional): <雑音>などのタグを含む発話を無視するかどうか. Defaults to True.
        isolate_extra_content (bool, optional): <雑音>などのタグを含む発話を別の発話として扱うかどうか. Defaults to True.
        para_info (dict, optional): <雑音>などのタグを含む発話を別のファイルに出力する際のファイル名. Defaults to {'F': 'filler', 'D': 'disfluency', '笑': 'laugh', 'W': 'ambiguity', 'M': 'meta'}.
    """
    parsed = parse_trn_file(trn_file_path, encoding=encoding)
    connected = connect_utterances(parsed, gap=gap, max=max, remove_extra_content=remove_extra_content, isolate_extra_content=isolate_extra_content)
    ...
    if wav_file_path:
        ...
        with open(filepath, 'w') as file:
            file.write('\n'.join(make_wav_scp(wav_file_path)) + '\n')
    # utt2spkの出力
    filepath = os.path.join(output_dir, "utt2spk")
    with open(filepath, 'w') as file:
        file.write('\n'.join(make_utt2spk(texts)) + '\n')
...
if __name__ == "__main__":
    ...
    main(args.trn_file_path, args.output_dir, args.mode, 
         wav_file_path=args.wav_file_path,
         encoding=args.encoding,
         gap=args.gap, max=args.max, 
         remove_extra_content=args.remove_extra_content,
         isolate_extra_content=args.isolate_extra_content, **kwargs)
